...
public class JobController {
    ...
    public JobController(SchedulerFactoryBean schedulerFactoryBean, DynamicJobService jobService, JobEntityRepository repository) {
    ...
    public String refresh(@PathVariable @NotNull String id) throws SchedulerException {
        ...
        synchronized (log) {
            ...
            if (entity.getStatus().equals("OPEN")) {
                ...
                result = "Refresh Job : " + entity.getName() + "\t jarPath: " + " success !";
            } else {
                result = "Refresh Job : " + entity.getName() + "\t jarPath: " + " failed ! , " +
    ...
    private void reStartAllJobs() throws SchedulerException {
        synchronized (log) {                                                         //只允许一个线程进入操作
            ...
            for (JobEntity job : jobService.loadJobs()) {                               //从数据库中注册的所有JOB
                log.info("Job register name : {} , group : {} , cron : {}", job.getName(), job.getJobQueue(), job.getCron());
